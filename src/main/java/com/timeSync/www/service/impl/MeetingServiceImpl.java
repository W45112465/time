...
//        }
    ...
    @Override
    public List<HashMap> searchUserTaskListByPage() {
        List<HashMap> list = meetingDao.searchUserTaskListByPage();
        list.forEach(c -> {
            c.put("processType", "meeting");
        });
        return list;
    }

    @Override
    public int approvalMeeting(HashMap param) {
        HashMap map = new HashMap();
        map.put("id", param.get("id"));
        map.put("status", param.get("approval").equals("同意") ? 3 : 2);
        System.out.println(param.get("approval").toString());
        return meetingDao.approvalMeeting(map);
    }

    @Override
    public List<HashMap> searchUserTaskListByPageTwo() {
        return meetingDao.searchUserTaskListByPageTwo();
    }
