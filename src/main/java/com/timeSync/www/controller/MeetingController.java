...
public class MeetingController {
    ...
    public R searchMyMeetingListByPage(@Valid @RequestBody SearchMyMeetingListByPageFrom from, @RequestHeader("token") String token) {
        int userId = jwtUtil.getUserId(token);
        Integer page = from.getPage();
        Integer length = from.getLength();
    ...
    @ApiOperation("查询未审批会议")
    public R searchUserTaskListByPage(@Valid @RequestBody SearchUserTaskListByPageForm form) {
        System.out.println(form.getCurrent());
        return R.ok().put("result", Integer.parseInt(form.getCurrent()) == 0 ? meetingService.searchUserTaskListByPage() : meetingService.searchUserTaskListByPageTwo());
    }
    ...
    public R approvalMeeting(@Valid @RequestBody ApprovalMeetingForm form) {
        ...
        map.put("approval", form.getApproval());
        meetingService.approvalMeeting(map);
        return R.ok();
    }
}
